...
//solves https://cses.fi/problemset/task/1684/

// 2-SAT (2-satisfiability problem)
// given boolean variables on the form (a or b) and (c or d) and... (y or z)
// is there a true/false assignment for each variable such that all constraints are satisfied?
//
// solve() returns a pair
// first = is possible?
// second =  vector[i] = 1 if i is true;
// Done in linear time
...
struct TWOsat {
	...
	TWOsat() {}
	TWOsat(int n_) : n(n_), tot(n), g(2*n) {}
  ...
  //All of these operations translates to x ou y in some way;
	...
	void add_or(int x, int y) { // x ou y
		add_impl(~x, y);
	}
